...
typedef std::shared_ptr<class AveragingNode>	AveragingNodeRef;
...
class AveragingNode : public audio::Node {
public:
	// Construct a new AveragingNode and pass in optional audio::Format parameters.
	...
	AveragingNode ( float * sharedBuff, const Format &format = Format () ) : Node ( format )	{ mSharedBuffer = sharedBuff; }
...
protected:
	...
	virtual void process ( audio::Buffer *buffer )	override;
